...
-- select * from TB_Comprobantes where nrComprobante  in ('00603887')

/*

  exec  dbo.spu_obtieneDatosCITIVentas_v4_9 1, 2015
  exec  dbo.spu_obtieneDatosCITIVentas_v4_9 2, 2015

  exec  dbo.spu_obtieneDatosCITIVentas_Alicuotas_v4_9 1, 2015
...
alter procedure  [dbo].[spu_obtieneDatosCITIVentas_Alicuotas_v4_9](@mes int, @anio int) 
as
begin
		...
		INSERT INTO RTMP_auxiliarPermisosRenglones (Renglon)
		Select  right( replicate('0',3) + dbo.UDF_obtenerCodDOC_CITIT_v4_7(tpComprobante , tpLetra ),3) + -- as  cod_citi,  -- Tipo de Comprobante
		...
		CASE  ABS(isnull(A.vlIVA,0)) when 0  THEN  dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( 0 , 15, 2)  ELSE dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( isnull(vlTotalGeneral,0) -  ABS(isnull(A.vlIVA,0)) , 15, 2) END  + --     As imp_neto_gravado,     --  importe total del operacion
		+ (case ABS(isnull(A.vlIVA,0)) when 0
		 then dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( 3 , 4, 0)
		else dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( 5 , 4, 0)  end) +
...
--  {Importe Total de la Operación} = {Importe Neto Gravado} + {Impuesto Liquidado} + {los demás importes del cpte}. 

-- exec spu_ControlCITIVentas_v4_9 @mes=2  , @anio=2015
-- drop procedure spu_ControlCITIVentas_v4_9

alter procedure spu_ControlCITIVentas_v4_9 (@mes int, @anio int) 
as
begin 

		Select 
		dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (vlTotalGeneral, 15, 2)         As imp_total,     --  importe total del operación
		CASE  ABS(isnull(A.vlIVA,0)) when 0  THEN  dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( 0 , 15, 2)  ELSE dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( isnull(vlTotalGeneral,0) -  ABS(isnull(A.vlIVA,0)) , 15, 2) END  As imp_neto_gravado,    
		dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (ABS(isnull(A.vlIVA,0)),15,2)   As impuesto_liquidado,	 -- impuesto de IVA
		CASE isnull(a.vlIVA,0) WHEN 0 THEN dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (vlTotalGeneral,15,2) ELSE dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (0 ,15,2) END as  importe_total_conceptos_no_neto_gravado		
		FROM TB_Comprobantes A 
		WHERE month(A.dtComprobante) = @mes    and year(A.dtComprobante) = @anio 
		AND tpLetra <>'X' 
		/* AND vlTotalGeneral = ( CASE  ABS(isnull(A.vlIVA,0)) when 0  THEN   0 ELSE  isnull(vlTotalGeneral,0) -  ABS(isnull(A.vlIVA,0))  END   + --  imp_neto_gravado,    
								ABS(isnull(A.vlIVA,0))   +  	 --  impuesto_liquidado impuesto de IVA
								CASE isnull(a.vlIVA,0) WHEN 0 THEN vlTotalGeneral ELSE 0 END ) -- importe_total_conceptos_no_neto_gravado		

		*/
		AND nrTalonario  = '0006'
		AND nrComprobante = '00603887'
...
---  exec  dbo.spu_obtieneDatosCITIVentas_v4_9 1, 2015
alter procedure  [dbo].[spu_obtieneDatosCITIVentas_v4_8](@mes int, @anio int) 
as
begin
		...
		*/ 
		...
		--vlTotalGeneral,
		+ dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (vlTotalGeneral, 15, 2)   --    As imp_total,     --  importe total del operacion
		+ CASE isnull(a.vlIVA,0) WHEN 0 THEN dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (vlTotalGeneral,15,2) ELSE dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (vlTotalGeneral - a.vlIVA ,15,2) END --  ,	--  importe total conceptos que no integran el neto gravado		
		+ dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (0,15,2) -- As imp_perc_no_cate,						--  percepcion a no categorizados
		+ dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (0,15,2) -- As imp_exen,						--  importe operaciones exentas
		+ dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (0,15,2) -- As imp_perc_ctas ,				--  importe percepciones o pagos a ctas impuestos nacionales
		+ dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (0,15,2) -- As imp_perc_iibb ,				--  importe percepciones de ingresos brutos
...
alter procedure  [dbo].[spu_obtieneDatosCITIVentas_Alicuotas_v4_7](@mes int, @anio int) 
as
begin
		...
		+ (case ABS(isnull(A.vlIVA,0)) when 0
		 then dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( 3 , 4, 0)
		else dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( 4 , 4, 0)  end) +
		-- dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( 4 , 4, 0)    + --  As alicuota_iva,     --  importe total del operacion
		dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (ABS(isnull(A.vlIVA,0)),15,2)  -- As impuesto_liquidado				--  importe total conceptos que no integran el neto gravado		
		FROM TB_Comprobantes A 
		WHERE month(A.dtComprobante) = @mes    and year(A.dtComprobante) = @anio 
