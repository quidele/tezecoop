...
begin
		...
		dbo.UDF_obtenerFormatoNumericoAFIP (ABS(isnull(A.vlSubtotal,0)),15,2) As imp_iva,				--  importe total conceptos que no integran el neto gravado		
		...
		*/ 
		...
		+ dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 (ABS(isnull(A.vlSubtotal,vlTotalGeneral)),15,2) --  As imp_iva,				--  importe total conceptos que no integran el neto gravado		
...
begin
		...
		dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( isnull(vlTotalGeneral,0) -  ABS(isnull(A.vlIVA,0)) , 15, 2)  + --     As imp_neto_gravado,     --  importe total del operacion
		+ (case ABS(isnull(A.vlIVA,0)) when 0
		 then dbo.UDF_obtenerFormatoNumericoAFIP_v4_7 ( 0 , 4, 0)
