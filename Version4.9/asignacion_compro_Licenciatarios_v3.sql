...
/*
	...
	/* obtenemos los licenciatarios con cuit ok */
		select  nrLicencia ,  nrCUIT ,   dbo.ufn_ValidarCUIT(nrCUIT) cuitOK,  nmNombre +' '+
		 nmApellido nombre_completo
...
*/
		...
		IF EXISTS ( SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.	and nrLicenciaProveedor not in (999,87) ') )
			begin
			drop table dbo.tmp_licenciaratios_CuitOK20016
			end
		GO
		...
		into tmp_licenciaratios_CuitOK20016
		from TB_Proveedores  where nrLicencia is not null  and  dbo.ufn_ValidarCUIT(nrCUIT) = 1 
								   order by nrLicencia
		*/

		IF EXISTS ( SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.tmp_comprobantes_sin_cuit20016') )
			begin
			drop table dbo.tmp_comprobantes_sin_cuit20016
			end
		GO
		...
		into tmp_comprobantes_sin_cuit20016
		FROM TB_Comprobantes A  inner join TB_Proveedores B  on  A.nrLicencia = b.nrLicencia 
		WHERE  year(A.dtComprobante) = 2016 
		AND  vlTotalGeneral   > 999  and   dbo.ufn_ValidarCUIT (A.nrDoc )= 0
		AND tpComprobante  <>'X'

		--- select * from tmp_comprobantes_sin_cuit20016 order by nrLicenciaProveedor 


		alter table tmp_comprobantes_sin_cuit20016 add idtabla int identity(1,1)

		--select * from tmp_comprobantes_sin_cuit20016
		/****************************************************************************/
		/***** REPROCESAR															*/ 
		/****************************************************************************/
		...
		FROM tmp_comprobantes_sin_cuit20016 where nrLicenciaCliente = '000000'
		ORDER BY nrLicenciaProveedor
		...
		BEGIN
			...
			from    tmp_comprobantes_sin_cuit20016
			...
			update  tmp_comprobantes_sin_cuit20016 set	nrLicenciaCliente = @nrLicenciaCliente, 
		...
		select * from tmp_comprobantes_sin_cuit20016 where nrLicenciaCliente = 87


			select * from tmp_comprobantes_sin_cuit20016 where nrLicenciaCliente = 124

			20-08629659-5	PASCUAL GILIBERTI


		select x.nrDoc , y.nrCUITCliente , x.dsRazonSocial , y.nombre_completo_cliente from TB_Comprobantes  x inner join   tmp_comprobantes_sin_cuit20016 y 
						on x.tpComprobante = y.tpComprobante
							and  x.tpLetra = y.tpLetra
								and  x.nrTalonario = y.nrTalonario
								and  x.nrComprobante = y.nrComprobante


				select x.* into  xTB_Comprobantes_asig_SIAP2016 from TB_Comprobantes  x inner join   tmp_comprobantes_sin_cuit20016 y 
						on x.tpComprobante = y.tpComprobante
							and  x.tpLetra = y.tpLetra
								and  x.nrTalonario = y.nrTalonario
								and  x.nrComprobante = y.nrComprobante

update x set  x.nrDoc = y.nrCUITCliente , x.dsRazonSocial =  y.nombre_completo_cliente from TB_Comprobantes  x inner join   tmp_comprobantes_sin_cuit20016 y 
						on x.tpComprobante = y.tpComprobante
							and  x.tpLetra = y.tpLetra
								and  x.nrTalonario = y.nrTalonario
								and  x.nrComprobante = y.nrComprobante