...
select count(*) from   [dbo].[TB_Comprobantes] 
where dtComprobante_hora is null  and year(dtComprobante) = 2015
...
---- Realizados antes del 31/12/2015 23:59:59 yCompensados despues de esa fecha 
...
WHERE     (TB_Cupones.flAnulado = 0) AND (TB_Comprobantes.dtComprobante_hora <= 
	  ...
	  select * from tb_comprobantes  where nrComprobante = '00192698' and nrTalonario = '0001'
...
FROM         TB_Cupones INNER JOIN
                      ...
                      TB_Cupones.tpComprobanteCliente = TB_Comprobantes.tpComprobante AND TB_Cupones.tpLetraCliente = TB_Comprobantes.tpLetra INNER JOIN
                      TB_Cajas ON TB_Comprobantes.nrCaja = TB_Cajas.nrCaja
WHERE     (TB_Cupones.flAnulado = 0)  AND (TB_Cupones.flCompensado = 0) AND 
	  year(dtComprobante)=2015