VERSION 5.00
Begin VB.Form frm_sincronizarPuesto 
   Appearance      =   0  'Flat
   BackColor       =   &H00FFC0C0&
   ...
   ClientTop       =   345
   ClientWidth     =   7380
   Icon            =   "frm_sincronizarPuesto.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   3225
   ...
   End
End
Attribute VB_Name = "frm_sincronizarPuesto"
...
salir_error:
    ...
    dsError = Err.Description + " - Función: asignarDatosObjetosSincronizador - Origen: " + Err.Source
...
Dim resp                        As Integer
    ...
    resp = MsgBox("Esta seguro que desea realizar la sincronizacion?", vbQuestion + vbYesNo, "Atención")
    ...
    If objUnidadesMoviles.CopiarMDBPendriveaDisco(Me.cbUnidadesRemovibles.Text) Then
        If Not Configurando_MDB(App.Path + "\SincroManual\Puesto_ADM.mdb") Then
            ...
            MsgBox "No se pudo sincronizar la información de ADM a Puesto." + vbCrLf + "Error: " + objUnidadesMoviles.Error, vbInformation, "Atencion"
        Else
    ...
    ' realizar sincronizacion
     If Not objSincronizadorManual.Sincronizar(l_strWherecajasaSincronizar) Then
        ...
        objUnidadesMoviles.CopiarMDBaPendrive (Me.cbUnidadesRemovibles.Text)
    ...
    MsgBox "Proceso Finalizado. Envíe el Pendrive a la Administración.", vbInformation + vbDefaultButton1, "Atención"
    Screen.MousePointer = vbDefault
    Me.Enabled = True
    Unload Me
