    ...
    public class ServiceConciliacionTodoPago : ServiceConciliacion
    {
            ...
            using (var context = new dbSG2000Entities())
            {
                using (TransactionScope transaction = new TransactionScope())
                {
                    ...
                    foreach (var detalleConciliacion in plistaDetalleConciliacion)
                    {
                        ...
                        // nueva logica para Todo Pago, se cobra a partir de la fecha de cupon mas 10 días
                        ...
                        var nrFactura = un_Cupon.tpComprobanteCliente + "-" + un_Cupon.tpLetraCliente + "-" +
                            un_Cupon.nrTalonarioCliente + un_Cupon.nrComprabanteCliente.Trim();

                        unSMC.GrabarAsientoContablePosdatados(un_Cupon.vlMontoAcreditacion.Value, objConciliacion.nrCajaAdm.Value,
                            objConciliacion.dsUsuario, objConciliacion.IdConciliacion.ToString(), context, Conciliacion_de_Viajes, 
                            Viajes_con_Tarjeta_a_Bancos, un_Cupon.nrLicencia.ToString(), nrFactura, detalleConciliacion.fechaPago.Value, un_Cupon.nrCupon, "Todo Pago");
        ...
        public override IEnumerable<Object> ObtenerDetalleConciliacion(long pId)
        {
            ...
            using (var context = new dbSG2000Entities())
            {
                ...
                var listadeViajesaConciliar1 = (from c in context.TB_Cupones
                                                join x in context.TB_ConciliacionDetalle
                                                      ...
                                                      TB_Conciliacion objConciliacion)
        {
            ...
            using (var context = new dbSG2000Entities())
            {
                ...
                var listaTB_CuponesDesconciliados = (from c in context.TB_Cupones where listaCuponesDesconciliados.Contains(c.nrCupon) 
                                                      select c.flCobradoalCliente).ToList(); ;
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    ...
                    foreach (var detalleConciliacion in listaDetalleDesconciliados)
                    {
                        ...
                        var nrFactura = un_Cupon.tpComprobanteCliente + "-" + un_Cupon.tpLetraCliente + "-" + un_Cupon.nrTalonarioCliente + "-" +
                            un_Cupon.nrComprabanteCliente.Trim() + "/ Cupon Todo Pago: " + ExtensionString.EmptyIfNull(un_Cupon.nrCuponPosnet).Trim();
                        ...
                        unSMC.GrabarAsientoContablePosdatados(vlMontoAcreditacion, objConciliacion.nrCajaAdm.Value,
                            objConciliacion.dsUsuario, objConciliacion.IdConciliacion.ToString(), context, Anula_Viajes_con_Tarjeta_a_Bancos, 
                            Anula_conciliacion_de_Viajes, un_Cupon.nrLicencia.ToString(), nrFactura, detalleConciliacion.fechaPago.Value, un_Cupon.nrCupon, "Todo Pago");
                ...
                using (TransactionScope transaction = new TransactionScope())
                {
                    ...
                    {   // eliminamos los detalle existentes
                        ...
                        var nrFactura = objCupon.tpComprobanteCliente + "-" + objCupon.tpLetraCliente + "-" +
                                        objCupon.nrTalonarioCliente + "-" + objCupon.nrComprabanteCliente.Trim() + "/ Cupon Todo Pago: " +
                                        ExtensionString.EmptyIfNull(objCupon.nrCuponPosnet).Trim();                                        
                        ...
                        unSMC.GrabarAsientoContablePosdatados(vlMontoAcreditacion, objConciliacion.nrCajaAdm.Value,
                                ...
                                nrFactura, item.fechaPago.Value, objCupon.nrCupon, "Todo Pago");
